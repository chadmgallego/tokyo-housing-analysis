#!/usr/bin/env python3
"""
Cron-friendly dashboard generator for Tokyo rental listings.

On each run it:
  1. Finds the most recent CSV in pipeline/dataset_results/
  2. Compares its filename to the one stored in .dashboard_state.env
  3. If the file is new (or state is missing), regenerates the dashboard
     and updates the stored filename
...
REPO_ROOT   = Path(__file__).parent.parent           # one level above pipeline/
DATASET_DIR = Path(__file__).parent / "dataset_results"  # pipeline/dataset_results/
OUTPUT_FILE = REPO_ROOT / "reports" / "tokyo_rental_dashboard.html"
PAGES_FILE  = REPO_ROOT / "docs" / "index.html"      # served by GitHub Pages
STATE_FILE  = REPO_ROOT / ".dashboard_state.env"
STATE_KEY   = "LAST_CSV_FILE"                         # tracks filename, not mtime
...
def get_latest_csv() -> Path | None:
    """Return the most recently modified CSV in dataset_results/, or None if empty."""
    csvs = sorted(DATASET_DIR.glob("*.csv"), key=lambda f: f.stat().st_mtime, reverse=True)
        ...
        for row in reader:
            try:
                ...
                rows.append({
                    ...
                    "anomaly_label": int(float(row["anomaly_label"])) if row.get("anomaly_label") else None,
                    "anomaly_score": round(float(row["anomaly_score"]), 4) if row.get("anomaly_score") else None,
                })
...
def compute_stats(rows: list[dict]) -> dict:
    ...
    for r in rows:
        ...
        a = r["area"]
        if   a < 20: area_buckets["<20"].append(r["rent"])
        elif a < 25: area_buckets["20-25"].append(r["rent"])
        elif a < 30: area_buckets["25-30"].append(r["rent"])
        elif a < 40: area_buckets["30-40"].append(r["rent"])
        elif a < 50: area_buckets["40-50"].append(r["rent"])
        else:        area_buckets["50+"].append(r["rent"])

        age = r["age"]
        if   age < 5:  age_dist["<5"]    += 1
        elif age <= 10: age_dist["5-10"]  += 1
        elif age <= 20: age_dist["11-20"] += 1
        elif age <= 30: age_dist["21-30"] += 1
        elif age <= 40: age_dist["31-40"] += 1
        elif age <= 50: age_dist["41-50"] += 1
        else:           age_dist["50+"]   += 1
    ...
    # Count listings flagged as anomalies by Isolation Forest (label = -1)
    anomaly_count = sum(1 for r in rows if r.get("anomaly_label") == -1)
...
def generate_html(rows: list[dict], stats: dict, csv_name: str) -> str:
    ...
    fp_options = "\n".join(f'    <option value="{fp}">{fp}</option>' for fp in known_fps)

    # Unique stations sorted by name (for station filter)
    all_stations_sorted = sorted({r["station"] for r in rows if r.get("station")})
    station_options = "\n".join(f'    <option value="{s}">{s}</option>' for s in all_stations_sorted)

    total_fmt         = f"{stats['total']:,}"
    median_rent_fmt   = fmt_yen(stats["median_rent"])
    anomaly_count_fmt = f"{stats['anomaly_count']:,}"
    median_area_fmt   = f"{stats['median_area']:.1f} m²"
    median_age_fmt    = f"{round(stats['median_age'])} yrs"

    # Pre-compute walk_rent with string keys before entering the f-string —
    # a dict comprehension inside double-braces conflicts with f-string escaping
    walk_rent_json = json.dumps({str(k): v for k, v in stats["walk_rent"].items()}, ensure_ascii=False)
...
.anomaly-flag {{ color: #E24B4A; font-weight: 500; }}
.tbl-pagination {{
...
<div class="page-header">
  ...
  <div class="page-sub">{total_fmt} listings · {csv_name} · Generated {generated_at}</div>
</div>
...
{station_options}
  ...
  <label style="margin-left:8px;">Anomalies:</label>
  <select id="anomalyFilter" onchange="applyFilter()">
...
<div class="kpi-grid">
  ...
  <div class="kpi"><div class="kpi-label">Anomalies</div><div class="kpi-value" id="k-anomaly">—</div><div class="kpi-sub">flagged by model</div></div>
...
const ALL_WALK_RENT = {walk_rent_json};

const DEFAULTS = {{
  total:   "{total_fmt}",
  median:  "{median_rent_fmt}",
  anomaly: "{anomaly_count_fmt}",
...
function applyFilter() {{
  ...
  const an  = document.getElementById("anomalyFilter").value;
  ...
  let sub = RAW.filter(d => {{
    ...
    const agOk = ag === "all"
      ...
      || (ag === "old" && d.age > 20);
    const anOk = an === "all"
      || (an === "anomaly" && d.anomaly_label === -1)
      || (an === "normal"  && d.anomaly_label === 1);
    return fpOk && arOk && stOk && agOk && anOk;
  }});

  const hasFilter = fp !== "all" || ar !== "all" || st !== "all" || ag !== "all" || an !== "all";
...
function updateKPIs(sub, hasFilter) {{
  if (!hasFilter) {{
    ...
    document.getElementById("k-anomaly").textContent = DEFAULTS.anomaly;
  ...
  const anomalies = sub.filter(d=>d.anomaly_label === -1).length;
  document.getElementById("k-total").textContent   = sub.length.toLocaleString();
  document.getElementById("k-median").textContent  = fmtY(median(rents));
  document.getElementById("k-anomaly").textContent = anomalies.toLocaleString();
...
function updateCharts(sub, fp, ag, wk, hasFilter) {{
  ...
  const stationArr = Object.entries(stationMap)
    .map(([name,rents])=>{{return {{name,rent:mean(rents)}}}})
    .sort((a,b)=>b.rent-a.rent).slice(0,8);
...
const TABLE_COLS = [
  ...
  {{key:"anomaly_label", label:"Anomaly",   fmt:v=>v===-1 ? '<span class="anomaly-flag">⚠ Anomaly</span>' : (v===1 ? "Normal" : "—")}},
  {{key:"anomaly_score", label:"Score",     fmt:v=>v!=null ? v.toFixed(4) : "—"}},
];
...
def read_stored_filename() -> str | None:
    """Read the last-processed CSV filename from the state file."""
...
def write_stored_filename(filename: str) -> None:
    """Persist the CSV filename to the state file."""
    STATE_FILE.write_text(f"{STATE_KEY}={filename}\n")


def build_dashboard(csv_file: Path) -> None:
    ...
    html  = generate_html(rows, stats, csv_file.name)
    OUTPUT_FILE.parent.mkdir(exist_ok=True)
    OUTPUT_FILE.write_text(html, encoding="utf-8")
    PAGES_FILE.parent.mkdir(exist_ok=True)
    PAGES_FILE.write_text(html, encoding="utf-8")
    print(f"[{datetime.now().strftime('%H:%M:%S')}] Dashboard generated — {stats['total']:,} listings, {stats['anomaly_count']:,} anomalies")
...
if __name__ == "__main__":
    ...
    stored_name  = read_stored_filename()
    current_name = csv_file.name

    if stored_name is None:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] No prior state — running for the first time.")
        build_dashboard(csv_file)
        write_stored_filename(current_name)
    elif current_name != stored_name:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] New file detected ({stored_name} → {current_name}) — regenerating dashboard.")
        build_dashboard(csv_file)
        write_stored_filename(current_name)
    else:
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Already up to date ({current_name}) — nothing to do.")
